...
public class DetallarBonoServlet extends ServletSeguro {
	...
	private void guardar (HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			bonoDao.guardarDetalle(bono, items, usuarioWeb);
			
			request.getSession().removeAttribute("items");
			mensaje("El detalle del bono fue agregado exitosamente", request, response);
			...
			mensaje("Ocurrio un error al guardar el bono", request, response);
			detalle(request, response);
	...
	private void procesarArchivo(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			if (FileUpload.isMultipartContent(request)){
				...
				String folio = (String) params.get("folio");
				//if(folpas == "0")folpas=folio;
				//llr
    			//  BonoDTO bono = bonoDao.bonoWebPorFolio( Integer.parseInt(folpas), usuarioWeb);
				BonoDTO bono = bonoDao.bonoWebPorFolio( Integer.parseInt(folio), usuarioWeb);
				  //llrf
				  //llr
				  ...
				  for (int i=1; procesarArchivo && i<datosArchivo.size(); i++){
					  ...
					  if(prestadoresDao.EsCirugia(codigo)){
					     ...
					     cobroPabellon = Integer.parseInt(fila[4]);
					     codProfesional = Integer.parseInt(fila[6]);
					  }
					  
					  int tipoPrestacion = 0;
					  int grupo = ((Integer.parseInt(codigo))/100000);
                      int salaComun = 0;
					  if (grupo==2) salaComun = Integer.parseInt(fila[5]); // antes era 6
					  
					  if(hosAmb != 2){
					  	
					     tipoPrestacion = Integer.parseInt(fila[1]);
					  ...
					  if((hosAmb!= 2 && grupo == hosAmb)|| hosAmb==2){
						listaPrestaciones.add(new String[]{codigo, "nombre", "1"});
						
						  // int[]{ totalAporteDipreca, totalAporteSeguro, totalCopago };
						
						Date fechaAtencion = sdf.parse( TextUtil.completarDerecha( "00000000", fila[2]) );
						...
						int[] valores = prestadoresDao.copagoYAportesPorPrestador(CMC, prestador.getRutAcreedor().trim(), listaPrestaciones, fechaAtencion, tipoPrestacion, codProfesional, codContrato, salaComun, cobroPabellon, "0", usuarioWeb);
					    ...
					    try {
						  ...
						  nuevoItem.setCantidadAtenciones(new Integer(fila[3])); // era 4
						
						  if(fila[4]!= null){nuevoItem.setIncluyePabellon(fila[4]);} // era 5
						  ...
						  if(prestadoresDao.EsCirugia(codigo)) {
						  	nuevoItem.setCodProfesional(fila[6]); // era 7
...
//				llr fin if agregado 
	...
	throws ServletException, IOException {
		...
		try {
			...
			boolean huboErrores = false;
			int[] valores = prestadoresDao.copagoYAportesPorPrestador(CMC, prestador.getRutAcreedor().trim(), listaPrestaciones, fechaAtencion, tipoPrestacion, codProfesional, codContrato, pensionadoOSalaComun, cobroPabellon, "0", usuarioWeb);
			BonoItemDTO nuevoItem = new BonoItemDTO();
