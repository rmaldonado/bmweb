...
public class LoginServlet extends HttpServlet{
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		// Filtro por Referer: Deja pasar a usuarios que provengan de unas pocas
		// direcciones web seleccionadas
		
		String direccionesValidas[] = new String[]{"llatin","127.0.0.1","dipreca.cl","172.16.1.13","172.16.2.110" };

		String referer = request.getHeader("Referer");
		boolean vieneDireccionValida = false;
		// si el referer es null, no permito que pase
		if (referer == null){ usuarioWeb = null; }
		else {
		...
		if (!vieneDireccionValida){ usuarioWeb = null; }
		...
		if ( usuarioWeb != null ){
			...
			if ("admin09".equals(usuario)){ usuarioWeb.setRutEmisor("1"); }

			
			// 2006 - Si el nivelUsuario es 00 o 01, busco al usuario en la tabla de beneficiarios
			if ("00,01".indexOf(nivelUsuario) > -1){
				...
				// Recupero el CMC para restringir los bonos que puede obtener
			...
			if ("22".equals(nivelUsuario)){
				...
				// Si el habilitado no esta activo, no puede entrar a la aplicacion - 2006.04.21
				...
				// Si la ciudad del habilitado NO es Santiago, no puede hacer bonos en Santiago
			...
			if ("23".equals(nivelUsuario)|| "24".equals(nivelUsuario)|| "25".equals(nivelUsuario)){
				UsuarioWeb uwTemp = new UsuarioWeb(rutUsuario);
				uwTemp.setNombreUsuario(rutUsuario);
			...
			// Redirijo a una página que me lleva al menu de inicio de la aplicacion, para usuarios validos
