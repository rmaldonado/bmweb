	...
	<table class="tabla-borde-delgado" id="filtro-min" style="<%= (mostrarFiltros)? "display:none":"" %>">
		<tr class="encabezados-tabla">
			<td style="text-align:right">
				<a href="javascript:mostrar_filtros()" title="Mostrar filtros para refinar el listado de Bonos">
	...
	<form name="formulario" method="post" action="Bonos">
		...
		<tr class="fila-detalle-impar">
			...
			<td style="text-align:left">
				<select name="opfolio">
				<option value="no" <%= "no".equals(opfolio)?"selected":"" %>>No filtrar por Numero de Bono</option>
...
--%>
		<tr class="fila-detalle-impar">
			...
			<td style="text-align:left">
				...
				<option value="eq" <%= opemisor.equals("eq")?"selected":"" %>>Rut igual a</option>
...
%>
		<tr class="<%=clase%>">
			<td><a href="Bonos?accion=detalle&folio=<%= b.getFolio() %>" title="Ver detalle"><%= b.getFolio() %></a></td>
			<td><%= fechaEmision %></td>
			<td><%= tipoBono %></td>
			<td><%= b.getCarneBeneficiario() %></td>
			<td><%= b.getCodigoHabilitado() %></td>

			<td><a href="Bonos?accion=detalle&folio=<%= b.getFolio() %>">Ver Detalle</a></td>
			<td><a href="BonoPDF.pdf?folio=<%= b.getFolio() %>" target="_blank">Ver Archivo PDF</a></td>
			<td><a href="DetallarBonos?accion=detalle&folio=<%= b.getFolio() %>&op=nuevo" title="Ingresar Detalle de Prestaciones">Detallar</a> </td>
...
%>			
	...
	<p>
	<form method="post" action="Bonos">
	<input type="hidden" name="accion" value="crear">
	<input type="submit" value="Crear un Bono Abierto" class="submit">
	</form>
	</p>
	...
	<script language="javascript">
	  ...
	  function anular(folio){
	    if (confirm("Confirme que desea anular el siguiente bono: ''" + folio + "'' ")){
	       document.location = "Bonos?accion=anular&folio=" + folio;
	    }
	  }
	  ...
	  function limpiar_filtros(){
		...
		//document.formulario.paramTipoBono.selectedIndex = 0;
